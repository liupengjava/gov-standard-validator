...
export function buildKnowledgeSliceProgressMessage(progress: number, sliceCount = 0): string {
  if (progress >= 100) return `切分完成，已生成 ${sliceCount} 个条款切片`;
  if (progress >= 82) return "正在生成条款级知识切片";
  if (progress >= 55) return "正在识别条款编号和层级结构";
  if (progress >= 20) return "正在准备待切分文本";
  return "等待自动切分";
}

export function buildKnowledgeParseProgressMessage(progress: number): string {
  if (progress >= 100) return "解析完成，可自动切分";
  if (progress >= 72) return "正在清洗正文结构";
  if (progress >= 35) return "正在抽取文档正文";
  if (progress >= 10) return "正在准备解析任务";
  return "等待解析文件";
}
...
function isDraftTableOfContentsLine(line: string): boolean {
  const trimmed = line.trim().replace(/\s+/g, " ");
  if (!trimmed) return false;
  if (/^目\s*录$/.test(trimmed) || /\.{4,}|…{2,}/.test(trimmed)) return true;
  if (/^\d{1,2}\s+.+\s+\d{1,3}$/.test(trimmed) && !/^\d+(?:\.\d+)+/.test(trimmed) && trimmed.length <= 48) return true;
  if (/^附\s*录\s+[A-ZＡ-Ｚ]\s+.+\s+\d{1,3}$/.test(trimmed) && trimmed.length <= 80) return true;
  return false;
}
...
export function sliceDraftTextForReview(text: string, sourceName = "待验证文本"): DraftTextSlice[] {
  ...
  const lines = cleanText
    .split(/\n+/)
    .map((line) => line.trim())
    .filter((line) => line && !isDraftTableOfContentsLine(line));
...
export function updateDraftTextSlice(
  slices: DraftTextSlice[],
  statuses: Record<string, DraftTextSliceStatus>,
  sliceId: string,
  patch: { title: string; text: string }
): { slices: DraftTextSlice[]; statuses: Record<string, DraftTextSliceStatus> } {
  let changed = false;
  const nextSlices = slices.map((slice) => {
    ...
    return {
      ...slice,
      title: patch.title.trim() || slice.title,
      text,
      charCount: text.length,
      status: "pending" as const,
    };
  });
  if (!changed) return { slices, statuses };
  const { [sliceId]: _removed, ...nextStatuses } = statuses;
  return { slices: nextSlices, statuses: nextStatuses };
}

export function removeDraftTextSlice(
  slices: DraftTextSlice[],
  statuses: Record<string, DraftTextSliceStatus>,
  sliceId: string
): { slices: DraftTextSlice[]; statuses: Record<string, DraftTextSliceStatus> } {
  const nextSlices = slices.filter((slice) => slice.id !== sliceId);
  if (nextSlices.length === slices.length) return { slices, statuses };
  const { [sliceId]: _removed, ...nextStatuses } = statuses;
  return { slices: nextSlices, statuses: nextStatuses };
}
...
export type KnowledgeCatalogSearchTask = {
  ...
  matchedTitle?: string;
  message: string;
  searchMode?: "catalog" | "fuzzy";
  query?: string;
  candidateType?: string;
};

function normalizeStandardSearchName(fileName: string): string {
  return fileName
...
const OFFICIAL_STANDARD_DETAIL_URLS: Record<string, string> = {
  "GB/Z24294.3-2017": "https://openstd.samr.gov.cn/bzgk/std/newGbInfo?hcno=58C700CFB8D4F0A5E0CC681D9D7453BD",
  "GB/T39554.1-2020": "https://openstd.samr.gov.cn/bzgk/std/newGbInfo?hcno=C72648A217E6E5E8E4EB074CB6EE0E9B",
};

function standardLookupKey(fileName: string): string {
  return normalizeStandardSearchName(fileName).replace(/\s+/g, "").toUpperCase();
}

function buildOfficialStandardDetailUrl(fileName: string): string | undefined {
  return OFFICIAL_STANDARD_DETAIL_URLS[standardLookupKey(fileName)];
}

function buildKnowledgeSearchResultTitle(fileName: string): string {
  return `国家标准 ${normalizeStandardSearchName(fileName)}`;
}
...
export function buildKnowledgeCatalogSearchTasks(catalog: string, addedAt: string): KnowledgeCatalogSearchTask[] {
  return catalog
    ...
    .map((fileName, index) => {
      ...
      const officialDetailUrl = buildOfficialStandardDetailUrl(fileName);
      return {
        id: `KCS-${String(index + 1).padStart(3, "0")}`,
        fileName,
        status: "待检索",
        searchUrl: `https://www.baidu.com/s?wd=${encodeURIComponent(query)}`,
        searchResultUrl: officialDetailUrl,
        searchResultTitle: buildKnowledgeSearchResultTitle(fileName),
        detailPageUrl: officialDetailUrl,
        downloadActionLabel: "下载标准",
        sourceSite: "互联网检索",
        addedAt,
...
export function buildFuzzyKnowledgeSearchTasks(queryText: string, region: string, addedAt: string): KnowledgeCatalogSearchTask[] {
  const query = queryText.trim().replace(/\s+/g, " ");
  if (!query) return [];
  const normalizedRegion = region.trim();
  const candidateTypes = [
    { type: "国家标准", suffix: "相关国家标准" },
    { type: "行业标准", suffix: "相关行业标准" },
    { type: "地方标准", suffix: `${normalizedRegion || "地方"}相关地方标准` },
    { type: "政策文件", suffix: `${normalizedRegion || "地方"}政策文件` },
    { type: "办事指南", suffix: `${normalizedRegion || "地方"}政务服务办事指南` },
  ];
  return candidateTypes.map((candidate, index) => {
    const searchQuery = `${query} ${candidate.suffix} PDF`;
    const officialDetailUrl = buildOfficialStandardDetailUrl(searchQuery);
    return {
      id: `KFS-${String(index + 1).padStart(3, "0")}`,
      fileName: `${query} - ${candidate.suffix}`,
      status: "待检索" as const,
      searchUrl: `https://www.baidu.com/s?wd=${encodeURIComponent(searchQuery)}`,
      searchResultUrl: officialDetailUrl,
      searchResultTitle: `${candidate.type}公开来源：${query}`,
      detailPageUrl: officialDetailUrl,
      downloadActionLabel: candidate.type.includes("标准") ? "下载标准" : "下载原文",
      sourceSite: "缺失知识模糊检索",
      addedAt,
      message: `候选类型：${candidate.type}。系统将按模糊条件检索多个公开文件，命中后可分别加入知识库。`,
...
export function buildRetrievedKnowledgeDocumentPreviewUrl(task: KnowledgeCatalogSearchTask, documentName: string, capturedAt: string): string {
  const title = documentName || task.matchedTitle || task.fileName;
  const resultTitle = task.searchResultTitle || task.matchedTitle || title;
  const resultUrl = task.searchResultUrl || task.detailPageUrl || task.searchUrl;
  const detailUrl = task.detailPageUrl || resultUrl;
  const downloadLabel = task.downloadActionLabel || "下载原文";
...
<head>
  ...
  <style>
    body{font-family:Arial,"Microsoft YaHei",sans-serif;margin:32px;line-height:1.75;color:#14304f;background:#f7fcff}
    main{max-width:920px;margin:auto;background:#fff;border:1px solid #d8e9f7;border-radius:12px;padding:28px}
    h1{font-size:22px;margin:0 0 12px}
    .meta{color:#5f7891;font-size:13px;margin-bottom:20px}
    .badge{display:inline-block;border:1px solid #bde6ff;background:#edf8ff;color:#075ec9;border-radius:999px;padding:2px 10px;font-size:12px}
    section{border-top:1px solid #e6eef6;margin-top:20px;padding-top:18px}
...
<body>
<main>
  <span class="badge">已获取文档预览</span>
  <h1>${escapeHtmlText(title)}</h1>
  <div class="meta">采集时间：${escapeHtmlText(capturedAt)} ｜ 来源：${escapeHtmlText(task.sourceSite)} ｜ 检索任务：${escapeHtmlText(task.id)}</div>
  <section>
    <h2>文档来源说明</h2>
    <p>系统已按检索任务命中并形成知识文件预览。本页面用于查看已获取的原文文档，不再直接跳转搜索引擎结果页。</p>
    <ol>
      <li>打开百度搜索结果：<a href="${escapeHtmlText(task.searchUrl)}">${escapeHtmlText(task.searchUrl)}</a></li>
      <li>点击搜索结果“${escapeHtmlText(resultTitle)}”：<a href="${escapeHtmlText(resultUrl)}">${escapeHtmlText(resultUrl)}</a></li>
      <li>进入国家标准全文公开系统详情页：<a href="${escapeHtmlText(detailUrl)}">${escapeHtmlText(detailUrl)}</a></li>
      <li>点击“${escapeHtmlText(downloadLabel)}”获取原文，并固化为当前可查看文档。</li>
    </ol>
  </section>
  <section>
    <h2>文档摘要</h2>
    <p>${escapeHtmlText(task.message || "已获取公开来源知识文件，可继续执行自动切分和向量构建。")}</p>
  </section>
</main>
</body>
</html>`;
...
export function isRetrievedKnowledgeDocumentPreviewUrl(url: string): boolean {
  return /^data:text\/html;charset=utf-8,/i.test(url);
}

export function decodeRetrievedKnowledgeDocumentPreviewUrl(url: string): string {
  if (!isRetrievedKnowledgeDocumentPreviewUrl(url)) return "";
...
export function paginateVectorKnowledgeClauses<T>(items: T[], requestedPage: number, pageSize = 20) {
...
export function paginatePublicSentimentVectorSamples<T>(items: T[], requestedPage: number, pageSize = 20) {
  const page = paginateVectorKnowledgeClauses(items, requestedPage, pageSize);
  return {
    items: page.items,
    page: page.page,
    ...
    total: page.total,
    startIndex: page.start ? page.start - 1 : 0,
    endIndex: page.end ? page.end - 1 : 0,
  };
}
...
function inferPublicSentimentIssueTags(text: string): string[] {
  const rules: Array<[RegExp, string]> = [
    [/材料|纸质|重复提交|补正/, "材料要求"],
    [/线上|线下|平台|自助|网站|同步/, "线上线下一致性"],
    [/进度|查询|状态|反馈/, "进度查询"],
    [/窗口|大厅|排队|标识|服务/, "窗口服务"],
    [/投诉|差评|评价|回访/, "评价反馈"],
    [/敏感|隐私|脱敏|安全|数据/, "数据安全"],
...
function signalMatchesValidationContext(signal: SignalSample, match: MatchResult): boolean {
  if (signal.matchedClauseId && signal.matchedClauseId === match.clause.id) return true;
  const text = `${signal.text} ${signal.evaluationText || ""}`;
  if (match.clause.keywords.some((keyword) => keyword && text.includes(keyword))) return true;
  return inferPublicSentimentIssueTags(text).some((tag) => tag.includes(match.clause.dimension) || match.clause.dimension.includes(tag.slice(0, 2)));
}

export function buildPublicSentimentSupport({ match, signals }: { match: MatchResult; signals?: SignalSample[] }): PublicSentimentSupport {
  const availableSignals = signals || [];
  const relatedSignals = availableSignals.filter((signal) => signalMatchesValidationContext(signal, match));
  const supportSignals = relatedSignals.length ? relatedSignals : availableSignals.slice(0, Math.min(3, availableSignals.length));
  const mergedText = supportSignals.map((signal) => `${signal.text} ${signal.evaluationText || ""}`).join(" ");
  const issueTags = uniqueNonEmpty(supportSignals.flatMap((signal) => inferPublicSentimentIssueTags(`${signal.text} ${signal.evaluationText || ""}`)));

  return {
    sampleCount: supportSignals.length,
    evidenceLevel: "辅助依据",
    relatedSources: uniqueNonEmpty(supportSignals.map((signal) => signal.source)),
    relatedTypes: uniqueNonEmpty(supportSignals.map((signal) => signal.type)),
    issueTags: issueTags.length ? issueTags : mergedText ? ["群众体验"] : [],
    summaries: supportSignals.slice(0, 3).map((signal) => signal.evaluationText || signal.text).filter(Boolean),
    boundaryNote: "群众感知佐证仅用于提示现实执行关注点、影响范围和复核优先级，不替代标准知识库比对结论。",
  };
}
...
export function buildFormattedVerificationReport({
  ...
  const sentimentSupport = buildPublicSentimentSupport({ match, signals });
  const sentimentRows = sentimentSupport.sampleCount
    ? [
        `关联样本数量：${sentimentSupport.sampleCount} 条。`,
        `来源类型：${sentimentSupport.relatedSources.join("、") || "未标注来源"}；样本类别：${sentimentSupport.relatedTypes.join("、") || "未标注类别"}。`,
        `典型问题标签：${sentimentSupport.issueTags.join("、") || "暂无明确标签"}。`,
        "典型摘要：",
        ...sentimentSupport.summaries.map((summary, index) => `${index + 1}. ${summary}`),
        `证据边界：${sentimentSupport.boundaryNote}`,
      ].join("\n")
    : [
        "暂无可用辅助样本。",
        "证据边界：群众感知佐证仅用于提示现实执行关注点、影响范围和复核优先级，不替代标准知识库比对结论。",
      ].join("\n");
...
置信度：${point.confidence}分；人工复核状态：${point.reviewStatus}；专家确认结果：${decision}。`;
  ...
  const sources = [
    ...
    `B01｜舆情与调研样本库｜${sentimentSupport.evidenceLevel}｜群众感知佐证、问题标签和复核优先级参考`,
...
主依据：${match.clause.source} / ${match.clause.id}
命中条款：${match.clause.text}
待验证表达：${match.targetText}
比对判断：综合置信度 ${match.score} 分，${match.conclusion}。
主要差异：${match.issues.join("；")}
主次说明：本报告的验证重点仍为待验证文本与现有标准知识库的条款级比对，群众感知只进入辅助佐证层。
...
export type AgentExecutionStepCard = {
  phase: string;
  status: "done" | "running" | "pending";
  summary: string;
  subActions: AgentExecutionLogLine[];
};

export const AGENT_LOG_PLAYBACK_DELAY_MS = 280;

const DEFAULT_AGENT_EXECUTION_PHASES = ["读取输入", "结构抽取", "规则校验", "知识库比对", "舆情感知比对", "结论生成"];
...
export function buildAgentExecutionTrace({
  ...
  return [
    ...
    {
      phase: "舆情感知比对",
      action: `关联 ${sentimentSupport.sampleCount} 条群众感知样本，提取 ${sentimentSupport.issueTags.length} 类问题标签。`,
      thought: `${sentimentSupport.evidenceLevel}：用于提示现实执行关注点和复核优先级，不改变标准知识库比对主结论。`,
      evidence: sentimentSupport.summaries[0] || "暂无可用辅助样本。",
      status: "done",
    },
    {
      phase: "结论生成",
...
export function buildAgentExecutionStepCards(
  auditLines: AgentExecutionLogLine[],
  {
    started,
    running,
    expectedPhases = DEFAULT_AGENT_EXECUTION_PHASES,
  }: {
    started: boolean;
    running: boolean;
  ...
  const usefulLines = auditLines.filter((line) => line.kind !== "system" && line.phase !== "任务完成");
  const grouped = new Map<string, AgentExecutionLogLine[]>();

  usefulLines.forEach((line) => {
    grouped.set(line.phase, (grouped.get(line.phase) || []).concat(line));
  });

  const lastActivePhase = [...usefulLines].reverse().find((line) => expectedPhases.includes(line.phase))?.phase || expectedPhases[0];

  return expectedPhases.map((phase) => {
    const subActions = grouped.get(phase) || [];
    const hasResult = subActions.some((line) => line.kind === "result");
    const hasStartedPhase = subActions.length > 0;
    const status: AgentExecutionStepCard["status"] = !started
      ? "pending"
      : running && phase === lastActivePhase && !hasResult
        ? "running"
        : hasStartedPhase && (!running || hasResult)
          ? "done"
          : "pending";
    const summary =
      subActions.find((line) => line.kind === "action")?.message ||
      subActions[0]?.message ||
      (started ? "等待上一阶段完成后执行。" : "等待点击开始文本验证。");

    return { phase, status, summary, subActions };
  });
}
...
export function buildAgentThinkingExecutionLog(input: {
  ...
  const sentimentSupport = buildPublicSentimentSupport({ match: input.match, signals: input.signals });
  const lines: Omit<AgentExecutionLogLine, "time">[] = [
    ...
    },
    {
      kind: "action",
      phase: "任务拆解",
      message: "任务拆解：将验证任务拆成文本切片复核、规则校验、知识库召回、舆情感知比对、差异判断、关键验证点生成和报告组装。",
    },
    {
      kind: "action",
      phase: "工具调用",
    ...
    },
    {
      kind: "evidence",
      phase: "证据读取",
      message: `证据读取：读取命中关键词 ${overlap}，主要参照 ${input.match.clause.id}，同步读取 ${sentimentSupport.sampleCount} 条群众感知辅助样本用于专家复核。`,
    },
    {
      kind: "reasoning",
      phase: "差异判断",
